...
let rec convert (e : expr) : lamcom = 
  let rec convert_var (e : expr) (s : Ast.var list) : lamcom = 
    ...
    | Uop(u,e') -> begin
        match u with
        | Hd -> let x = convert_var e' s in
          begin match x with
            | App (App (_,  Bool b), e2) when b = true -> x
            | _ -> App (Church.fst, (App (Church.snd, (convert_var e' s))))
          end
        | Tl -> let x = convert_var e' s in
          begin match x with
            | App (App (_,  Bool b), e2) when b = true -> x
            | _ -> App (Church.snd, (App (Church.snd, (convert_var e' s))))
          end
